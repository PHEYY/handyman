...
if (isset($_POST['submit'])) {
 $loc=$_POST['loc'];
 $queryn="select count(id) as number from customer  where location='$loc'";
 $resultn= mysqli_query($conn,$queryn);
 $rown=mysqli_fetch_array($resultn);

 $query1="select * from customer where location='$loc'";
  $result1=mysqli_query($conn,$query1);
  $rowcheck=mysqli_num_rows($result1);
  if ($rowcheck<1) {
    echo '<div id="rec">There are no records for customers in '.$loc.'</div>'; 
  }
  else{
   echo '<style>
      #receipt{
      ...
      <table><tr><th id="income">Customers in '.$loc.'</th></tr>
      <tr><th id="number"> number of customers</th><td i> '.$rown['number'].' customers</th></tr>
      </table>
