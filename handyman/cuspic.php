...
<?php 
$pic="select * from customer where user_name='$customerid'";
$res=mysqli_query($conn,$pic);
$rowp=mysqli_fetch_array($res);
$image=$rowp['image'];
if ($image) {
  echo '<form action="cuspic.php" method="post" enctype="multipart/form-data">
  <div><img src="/mafundi/handyman/MyUploadImages/'.$image.'" style="width:100px;height:100px;"/></div>
Change your profile Picture: <input type="file" name="file">
  ...
  echo'<form action="cuspic.php" method="post" enctype="multipart/form-data">
  <div><i class="fa fa-user" style="font-size:5em;"></i></div>
Change your profile Picture: <input type="file" name="file">
       <input type="submit" name="submit" value="Upload">
</form>
  ';
...
<form action="cuseditprofile.php" method="post" enctype="multipart/form-data">
...
if(file_exists(dirname($_SERVER['DOCUMENT_ROOT']).'/htdocs/mafundi/handyman/MyUploadImages/'.$name))
     ...
     else
     {
      ...
      $q= mysqli_query($conn,("update customer set image='$name' where user_name='$customerid'"));
  if($up && $q)
  {
