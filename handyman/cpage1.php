...
$queryloc="select location from customer";
...
if (isset($_POST['next'])) {
  ...
  $query= "select * from fundi where occupation= '$fundi' and location= '$custloc' and status='free'";
...
<input style="display:none;" type="text" name="payment" value="'.$payment.'">
<input style="display:none;" type="text" name="fundiid" value="'.$fundiid.'">
...
$mail=$row9["email"];
echo "<style> .fundiform{display:none;}</style>";
      echo '<form method="post" action="cpage1.php">
...
echo'<p><b>Mode of Payment:</b>  '.$payment.'</p>';
echo '<form method="POST" class="detail">
     ...
     <form method="POST" action="cpage1.php" class="detail">
...
$query1="insert into orders values('', $custom, $fundi, '$loc', '$task', '$payment', '$time',NULL,'TXNC001',NULL,NULL,NULL,NULL)";
  $result1=mysqli_query($conn, $query1);


  $query11 = "select o.date as date, o.order_id as orderid, c.email as cemail, c.first_name as cfname, c.last_name as clname, c.phone as cphone, f.first_name as ffname, f.last_name as flname, f.phone as fphone,f.email as femail, o.specific_location as olocation, o.task as task,  o.payment_mode as payment from customer c join orders o ON (c.id=o.customer_id) join fundi f on (o.fundi_id=f.id) where o.order_id=$orderid";
$result11=mysqli_query($conn,$query11);
$row11=mysqli_fetch_array($result11);
$date=$row11["date"];
...
<p><b>Mode of Payment:</b>$payment</p>
